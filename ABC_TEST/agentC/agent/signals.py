...
# Broader than _THEME_PATTERNS — used to restrict get_open_markets() to the
# geopolitics / international-relations universe shared by A/B/C.
_GEO_UNIVERSE_RE = re.compile(
    r"\b("
    r"iran|tehran|israel|gaza|lebanon|syria|yemen|houthi|hezbollah|hamas|netanyahu|"
    r"ukraine|russia|putin|zelensky|kyiv|kremlin|crimea|belarus|"
    r"taiwan|china|beijing|xi jinping|north korea|kim jong|"
    r"nato|pentagon|military|troops|invasion|ceasefire|airstrike|missile|drone strike|"
    r"war\b|conflict|sanction|embargo|hormuz|strait|"
    r"trump|biden|white house|congress|senate|president|prime minister|parliament|"
    r"election|ballot|referendum|"
    r"crude|brent|wti|oil price|opec|barrel|"
    r"geopolit|foreign policy|diplomat|treaty|annex"
    r")\b",
    re.I,
)


def is_geopolitics_market(*texts: str | None) -> bool:
    """True when market question/event text looks geopolitical (ABC + geo agent)."""
    blob = " ".join(t.strip() for t in texts if t and str(t).strip())
    if not blob:
        return False
    if _GEO_UNIVERSE_RE.search(blob):
        return True
    return classify_theme(blob) != "other"
