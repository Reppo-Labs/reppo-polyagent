...
def _pm_position_size(token_id: str) -> float | None:
    """
    Cross-check the Polymarket data API for an active position on this token.
    Returns share count if found with value > 0, otherwise None.

    This is the safety net for builder-API orders that are live on Polymarket
    but return 404 from CLOB get_order (the two systems can fall out of sync).
    We never close a DDB row as order_not_found when PM still shows the position.
    """
    ...
    try:
        r = _req.get(
            f"https://data-api.polymarket.com/positions?user={portfolio.lower()}",
            timeout=8,
        ...
        for p in r.json():
            ...
            if asset == token_id and float(p.get("currentValue") or 0) > 0.01:
                return float(p.get("size") or p.get("shares") or 0)
    except Exception as exc:
        logger.warning("PM data API check failed for %s: %s", token_id[:12], exc)
    return None
    ...
    for pos in ddb.get_open_or_pending_positions():
        ...
        try:
            ...
            if "404" in err or "not found" in err or "does not exist" in err:
                # Before closing: verify position isn't still live on Polymarket.
                # Builder-API orders can return 404 from get_order yet still exist
                # as filled positions on the PM backend — closing them in DDB would
                # make the agent blind to real open risk.
                pm_size = _pm_position_size(pos["token_id"])
                if pm_size is not None and pm_size > 0:
                    logger.info(
                        "order_not_found from CLOB but PM shows %.4f shares live for %s — keeping open",
                        pm_size, pos["token_id"][:12] + "…",
                    )
        ...
        if not isinstance(order, dict) or order.get("error"):
            ...
            if pm_size is not None and pm_size > 0:
                logger.info(
                    "CLOB order error but PM shows %.4f shares live for %s — keeping open",
                    pm_size, pos["token_id"][:12] + "…",
                )
                ddb.update_position_fill(
                    token_id=pos["token_id"],
